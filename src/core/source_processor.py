...
class SourceProcessor:
    ...
    def parse_uri(self, source_uri: str) -> Tuple[Literal["file", "http", "git"], Dict[str, Any]]:
    ...
    def _parse_file_uri(self, parsed: ParseResult) -> Tuple[Literal["file"], Dict[str, Any]]:
        """Parse file:// URI."""
    ...
    def _parse_https_uri(self, parsed: ParseResult) -> Tuple[Literal["http", "git"], Dict[str, Any]]:
        ...
        # Check if it's a JAR file (.jar suffix)
            ...
            raise ValueError("HTTPS/HTTP URIs must end with .jar (JAR file) or .git (Git repository)")
    
    def _parse_ssh_git_uri(self, source_uri: str) -> Tuple[Literal["git"], Dict[str, Any]]:
    ...
    def process_source(
        ...
        if uri_type == "file":
            ...
            return self._process_git_source(group_id, artifact_id, version, parsed_info, git_ref)
        else:
